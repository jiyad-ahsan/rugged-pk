---
type: "field-card"
slug: "fc-go-bag-packing"
title: "Go-Bag Packing List"
subtitle: "A complete packing reference for your 72-hour bag."
tag: "essentials"
date: "2026-04"
readTime: "2 min"
sketch: "backpack"
excerpt: "A single-page reference for packing a 72-hour go-bag. Print it, tape it inside your bag, and check it every 3 months."
relatedGuides: ["go-bag-guide", "30-minute-window"]
status: "published"
---

## Essentials (grab first)

- **Cash:** Rs. 20,000–50,000 in mixed denominations (ATMs go offline early)
- **Documents pouch:** see FC: Document Pouch for the full list
- **Phone charger + power bank:** fully charged, 20,000mAh minimum
- **Medications:** 7-day supply of any daily prescriptions
- **Water:** 3 litres minimum per person

## Food (72 hours)

- Sattu packets (3–4)
- Roasted chana (1 kg)
- Dates or gur (500g)
- Glucose biscuits (2 packs)
- Electrolyte sachets (ORS, 6 packets)

## Clothing & shelter

- 1 change of clothes (season-appropriate)
- Lightweight rain poncho or plastic sheet
- Dupatta or shemagh (sun, dust, warmth, bandage)
- Sturdy closed-toe shoes (worn, not packed)

## Tools

- LED torch + spare batteries
- Lighter + waterproof matches
- Multi-tool or sturdy knife
- Whistle
- Roll of duct tape
- 10m paracord or strong rope
- Permanent marker + zip-lock bags

## Medical

- Basic first aid kit (bandages, antiseptic, painkillers)
- Tourniquet (if trained, see FC: Tourniquet)
- Sanitary supplies
- Insect repellent
- Sunscreen

## Comms

- Walkie-talkie (if your family has them, see FC: Walkie Quickstart)
- Written list of emergency phone numbers (not just on your phone)
- Paper map of your city and surrounding areas

## Rotation check (every 3 months)

1. Replace expired food and medicine
2. Update cash amount
3. Charge the power bank
4. Check torch batteries
5. Update document copies if anything changed
